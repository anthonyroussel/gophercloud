...
// Use Extract to convert its result into an Workbook.
...
type ListOptsBuilder interface {
	ToWorkbookListQuery() (string, error)
}

// ListOpts filters the result returned by the List() function.
type ListOpts struct {
	// Scope filters by the workbook's scope.
	// Values can be "private" or "public".
	Scope string `q:"scope"`
	// CreatedAt allows to filter by workbook creation date.
	CreatedAt *ListDateFilter `q:"-"`
	// UpdatedAt allows to filter by last execution update date.
	UpdatedAt *ListDateFilter `q:"-"`
	// Name allows to filter by workbook name.
	Name *ListFilter `q:"-"`
	// Tags allows to filter by tags.
	Tags []string
	// Definition allows to filter by workbook definition.
	Definition *ListFilter `q:"-"`
	// Namespace allows to filter by workbook namespace.
...
// ToWorkbookListQuery formats a ListOpts into a query string.
func (opts ListOpts) ToWorkbookListQuery() (string, error) {
	q, err := gophercloud.BuildQueryString(opts)
	if err != nil {
		return "", err
	}
...
func List(client *gophercloud.ServiceClient, opts ListOptsBuilder) pagination.Pager {
	url := listURL(client)
	if opts != nil {
		query, err := opts.ToWorkbookListQuery()
		if err != nil {
	...
	return pagination.NewPager(client, url, func(r pagination.PageResult) pagination.Page {
		return WorkbookPage{pagination.LinkedPageBase{PageResult: r}}
	})
}
